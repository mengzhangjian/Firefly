...
def parse_argument():
    ...
    parser.add_argument(
        "--split", type=str, default="validation", help="split of dataset to evaluate"
    )
    parser.add_argument(
        "--output_dir", type=str, default="ceval_output", help="output directory"
    )
    parser.add_argument(
        "--DATA_PATH", type=str, default="dataset/eval/ceval/dev", help="eval directory"
    )
...
class CEval:
    ...
    def run_single_task(self, task_name: str, shot: int, split: str):
        ...
        for data in tqdm(dataset[split]):
            ...
            if shot != 0:
                shuffled = dataset["validation"].shuffle()
            ...
            print("正确答案为: {} 模型预测答案为: {}".format(answer, data["answer"].strip().upper()))
            results.append(
                {
                    "prompt": prompt,
...
def main():
    ...
    ceval = CEval(model, tokenizer, args.output_dir, args.DATA_PATH)
